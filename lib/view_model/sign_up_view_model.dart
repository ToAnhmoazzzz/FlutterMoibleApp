...
class SignUpViewModel extends GetxController {
  ...
  void serviceCallSignUp() {
    ...
    if(txtUsername.value.text.isEmpty) {
      Get.snackbar(Globs.appName, "Vui lòng điền tên đăng nhập");
      return;
    }

    if ( !GetUtils.isEmail(  txtEmail.value.text )  ) {
      Get.snackbar(Globs.appName, "Vui lòng điền địa chỉ email");
      return;
    }

    if (txtPassword.value.text.length < 6) {
      Get.snackbar(Globs.appName, "Vui lòng điền mật khẩu hợp lệ ít nhất 6 kí tự");
      return;
    }
